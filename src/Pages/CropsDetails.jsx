...
const CropsDetails = () => {
    ...
    // ✅ Properly define fetchCrop with useCallback (prevents re-creation on every render)
    const fetchCrop = useCallback(async () => {
        try {
            setLoading(true);
            const res = await axiosSecure(`/crops/${id}`);
            setData(res.data);
            setAvailableQuantity(res.data.quantity); // update availableQuantity
            setLoading(false);
        } catch (err) {
    ...
    // Fetch crop when page loads or ID changes
    useEffect(() => {
        fetchCrop();
    }, [fetchCrop]);

    const { _id, crop_name, type, price_per_unit, unit, quantity, location, crop_image, description, owner } = data || {};
                    ...
                    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 hover:shadow-xl transition-shadow duration-300">
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            <p className="flex items-center text-gray-700">
                                ...
                                <span>{availableQuantity}</span> available
                            </p>
                            <p className="flex items-center text-gray-700">
